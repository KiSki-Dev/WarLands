...
namespace Voronoi2
{
	...
	public class Voronoi
	{
                                               ...
                                               double minX, double maxX, double minY, double maxY)
        {
            ...
            {
                ClipEdgeToBoundingBox(edge, minX, maxX, minY, maxY);
            }

            return allEdges;
		}

        private void ClipEdgeToBoundingBox(GraphEdge edge, double minX, double maxX, double minY, double maxY)
        {
            double x0 = edge.x1, y0 = edge.y1;
            double x1 = edge.x2, y1 = edge.y2;

            RegionCode outCode0 = ComputeRegionCode(x0, y0, minX, maxX, minY, maxY);
            RegionCode outCode1 = ComputeRegionCode(x1, y1, minX, maxX, minY, maxY);
            bool accept = false;

            while (true)
            {
                if ((outCode0 | outCode1) == 0) // Both points inside
                {
                    accept = true;
                    break;
                }
                else if ((outCode0 & outCode1) != 0) // Both points share an outside zone
                {
                    break;
                }
                else
                {
                    double x = 0, y = 0;
                    RegionCode outCodeOut = (outCode0 != 0) ? outCode0 : outCode1;

                    if (outCodeOut.HasFlag(RegionCode.Top))
                    {
                        x = x0 + (x1 - x0) * (maxY - y0) / (y1 - y0);
                        y = maxY;
                    }
                    else if (outCodeOut.HasFlag(RegionCode.Bottom))
                    {
                        x = x0 + (x1 - x0) * (minY - y0) / (y1 - y0);
                        y = minY;
                    }
                    else if (outCodeOut.HasFlag(RegionCode.Right))
                    {
                        y = y0 + (y1 - y0) * (maxX - x0) / (x1 - x0);
                        x = maxX;
                    }
                    else if (outCodeOut.HasFlag(RegionCode.Left))
                    {
                    ...
                    if (outCodeOut == outCode0)
                    {
                        x0 = x;
                        y0 = y;
                        outCode0 = ComputeRegionCode(x0, y0, minX, maxX, minY, maxY);
                    }
                    else
                    {
                        x1 = x;
                        y1 = y;
                        outCode1 = ComputeRegionCode(x1, y1, minX, maxX, minY, maxY);
                    }
                }
            }
            ...
            {
                // Line is outside and not visible
                edge.x1 = edge.y1 = edge.x2 = edge.y2 = 0;
            }
        }

        private RegionCode ComputeRegionCode(double x, double y, double minX, double maxX, double minY, double maxY)
        {
            RegionCode code = RegionCode.Inside;

            if (x < minX) // to the left of the rectangle
                code |= RegionCode.Left;
            else if (x > maxX) // to the right of the rectangle
